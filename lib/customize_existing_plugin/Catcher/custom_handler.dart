...
import 'package:fb_clone_app/repository/api/api_urls.dart';

class CustomHttpHandler extends ReportHandler {
  final Dio _dio = Dio();
  ...
  CustomHttpHandler(this.requestType, this.endpointUri,
      {this.headers = const {},
  ...
  Future<bool> _sendPost(Report error) async {
    try {
      ...
      Response response =
          await _dio.post(endpointUri.toString()+APIUrls.ErrorLog_APIUrl, data: json, options: options);
      _printLog(
